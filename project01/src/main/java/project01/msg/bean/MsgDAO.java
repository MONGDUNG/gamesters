...
public class MsgDAO extends DataBaseConnection{
    ...
    // 2. 받은 쪽지 목록 조회
    public List<MsgDTO> getReceivedMessages(String receive_nick) {
       
        List<MsgDTO> messages = new ArrayList<>();
        try{
        	conn = getOracleConnection();
        	sql = "SELECT msg_num, msg, send_nick, reg FROM msg WHERE receive_nick = ? AND receiver_deleted = 0 ORDER BY reg DESC";
        	pstmt = conn.prepareStatement(sql);
